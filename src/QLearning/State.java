...
/*
This class is for states. It takes BiPedBody as an input and creates an approximate state from the values of the BiPedBody
 */
...
public class State {
    ...
    public String toString() {
        ...
        for (double angle : jointAngles) {
            s = s + Math.round(Math.toDegrees(angle) / 10);
        }
        return s;
    }
    ...
    public boolean equals(Object o) {
        // equals method is found when collision are found when searching the hashset "states" in StateAnalyser
        State s = (State) o;
        int round = 10;

        //Checks degree compared to world
        if (Math.round(Math.toDegrees(this.worldAngle) / round) != Math.round(Math.toDegrees(s.worldAngle) / round)) {
